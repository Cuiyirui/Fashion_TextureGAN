...
class BaseModel():
    ...
    def init_data(self, opt, use_D=True, use_D2=True, use_E=True, use_vae=True, use_VGGF=True, use_Dl=True):
        ...
        self.criterionGAN = networks.GANLoss(mse_loss=not use_sigmoid, tensor=self.Tensor)                                # criterion GAN adversarial loss
        self.wGANloss = networks.wGANLoss(tensor=self.Tensor)                                                             # wGAN adversarial loss
    ...
    def test(self, z_sample):  # need to have input set already
        ...
        # mask
        self.Mask = Variable(self.input_Mask, volatile=True)
        # condition
        if self.opt.dataset_mode == 'aligned_2':
            condition=1
        elif self.opt.dataset_mode=='aligned_3':
        ...
        if self.opt.whether_encode_cloth and self.opt.which_image_encode=='contour':
            ...
            # cloth
            self.real_C = Variable(self.input_C, volatile=True)
            self.fake_B = self.netG.forward(condition, self.z)
